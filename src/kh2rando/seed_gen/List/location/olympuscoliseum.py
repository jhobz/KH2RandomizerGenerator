...
from ..inventory import magic, keyblade, ability, misc, report
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class OCLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_oc_keyblade(inv)
        )
        ...
        self.logic[NodeId.UnderworldEntrance][
            NodeId.UnderworldEntranceChests
        ] = keyblade_lambda
        self.logic[NodeId.CavernsEntrance][
            NodeId.CavernsEntranceChests
        ] = keyblade_lambda
        ...
        cerberus_cup_check = (
            lambda inv: ItemPlacementHelpers.ht1_check(inv)
            and ItemPlacementHelpers.ag1_check(inv)
            and ItemPlacementHelpers.pl1_check(inv)
        )
        goddess_of_fate_check = (
            lambda inv: cerberus_cup_check(inv)
            and pain_and_panic_check(inv)
        ...
        if not reverse_rando:
            ...
            self.logic[NodeId.Hydra][
                NodeId.AuronsStatue
            ] = ItemPlacementHelpers.oc2_check
            ...
            self.logic[NodeId.Hades][NodeId.ParadoxCups] = (
                lambda inv: ItemPlacementHelpers.need_forms(inv)
                and ItemPlacementHelpers.need_summons(inv)
            )
        else:
            self.logic[START_NODE][
                NodeId.UnderworldEntrance
            ] = ItemPlacementHelpers.oc1_check
            self.logic[NodeId.Zexion][
                NodeId.CerberusBonus
            ] = ItemPlacementHelpers.oc2_check
            ...
            self.logic[NodeId.Hydra][NodeId.ParadoxCups] = (
                lambda inv: ItemPlacementHelpers.need_forms(inv)
                and ItemPlacementHelpers.need_summons(inv)
            )
...
def make_graph(graph: LocationGraphBuilder):
    ...
    passage_chests = graph.add_location(
        NodeId.PassageChests,
        [
    ...
    inner_chamber_chests = graph.add_location(
        NodeId.InnerChamberChests,
        [
    ...
    cerberus_bonus = graph.add_location(
        NodeId.CerberusBonus,
        [
            item_bonus(5, CheckLocation.Cerberus, oc, vanilla=ability.Counterguard),
        ],
    )
    coliseum_map_popup = graph.add_location(
        NodeId.ColiseumMapPopup,
        [
            popup(338, CheckLocation.ColiseumMap, oc),
        ],
    )
    urns_bonus = graph.add_location(
        NodeId.UrnsBonus,
        [
            item_bonus(57, CheckLocation.Urns, oc, vanilla=ability.AerialDive),
        ],
    )
    underworld_entrance_chests = graph.add_location(
        NodeId.UnderworldEntranceChests,
        [
    ...
    caverns_entrance_chests = graph.add_location(
        NodeId.CavernsEntranceChests,
        [
    ...
    lost_road_chests = graph.add_location(
        NodeId.LostRoadChests,
        [
    ...
    atrium_chests = graph.add_location(
        NodeId.AtriumChests,
        [
    ...
    demyx = graph.add_location(
        NodeId.DemyxOlympusColiseum,
        [
            stat_bonus(58, CheckLocation.DemyxOlympusColiseum, oc),
            popup(
                529,
                CheckLocation.DemyxOcSecretAnsemReport5,
                oc,
                vanilla=report.AnsemReport5,
            ),
            popup(293, CheckLocation.OlympusStone, oc, vanilla=misc.OlympusStone),
        ],
    )
    lock_chests = graph.add_location(
        NodeId.TheLockChests,
        [
    ...
    pete = graph.add_location(
        NodeId.PeteOlympusColiseum,
        [
            item_bonus(
                6, CheckLocation.PeteOlympusColiseum, oc, vanilla=ability.TrinityLimit
            ),
        ],
    )
    hydra = graph.add_location(
        NodeId.Hydra,
        [
    ...
    pain_panic_cup = graph.add_location(
        NodeId.PainAndPanicCup,
        [
            popup(
                513,
                CheckLocation.PainPanicCupProtectBelt,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
            popup(
                540,
                CheckLocation.PainPanicCupSerenityGem,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
        ],
    )
    cerberus_cup = graph.add_location(
        NodeId.CerberusCup,
        [
            popup(
                515,
                CheckLocation.CerberusCupRisingDragon,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
            popup(
                542,
                CheckLocation.CerberusCupSerenityCrystal,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
        ],
    )
    titan_cup = graph.add_location(
        NodeId.TitanCup,
        [
            popup(
                514,
                CheckLocation.TitanCupGenjiShield,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
            popup(
                541,
                CheckLocation.TitanCupSkillfulRing,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
        ],
    )
    goddess_of_fate_cup = graph.add_location(
        NodeId.GoddessOfFateCup,
        [
            popup(
                516,
                CheckLocation.GoddessOfFateCupFatalCrest,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ...
            popup(
                517,
                CheckLocation.GoddessOfFateCupOrichalcumPlus,
                [oc, cups],
                invalid_checks=[itemType.TROPHY],
            ),
        ],
    )
    paradox_cups = graph.add_location(
        NodeId.ParadoxCups,
        [
            popup(
                518,
                CheckLocation.ParadoxCupsHadesCupTrophy,
                [oc, locationType.OCParadoxCup],
                invalid_checks=[itemType.TROPHY, itemType.FORM, itemType.SUMMON],
                vanilla=misc.HadesCupTrophy,
            ),
        ],
    )
    zexion = graph.add_location(
        NodeId.Zexion,
        [
    ...
    data_zexion = graph.add_location(
        NodeId.DataZexion,
        [
            popup(
                551, CheckLocation.DataZexionLostIllusion, [oc, locationType.DataOrg]
            ),
        ],
    )
