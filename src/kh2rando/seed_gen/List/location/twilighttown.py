...
from ..configDict import locationType
from ..inventory import keyblade, ability, report, form, misc
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class TTLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_tt_keyblade(inv)
        )
        ...
        self.logic[NodeId.UndergroundConcourse][
            NodeId.UndergroundConcourseChests
        ] = keyblade_lambda
        ...
        self.logic[NodeId.MansionDiningRoom][
            NodeId.MansionDiningRoomChests
        ] = keyblade_lambda
        self.logic[NodeId.MansionLibrary][NodeId.MansionLibraryChests] = keyblade_lambda
        self.logic[NodeId.MansionBasement][
            NodeId.MansionBasementChests
        ] = keyblade_lambda
        # making tower checks logically need 3 ice creams to prevent softlocking
        self.logic[NodeId.YenSidTower][NodeId.YenSidTowerChests] = (
            lambda inv: ItemPlacementHelpers.tt3_check(inv) and keyblade_lambda(inv)
        )
        self.logic[NodeId.YenSidTowerEntryway][NodeId.YenSidTowerEntrywayChests] = (
            lambda inv: ItemPlacementHelpers.tt3_check(inv) and keyblade_lambda(inv)
        )
        self.logic[NodeId.SorcerersLoft][NodeId.SorcerersLoftChests] = (
            lambda inv: ItemPlacementHelpers.tt3_check(inv) and keyblade_lambda(inv)
        )
        self.logic[NodeId.TowerWardrobe][NodeId.TowerWardrobeChests] = (
            lambda inv: ItemPlacementHelpers.tt3_check(inv) and keyblade_lambda(inv)
        )
        if not reverse_rando:
            self.logic[START_NODE][NodeId.OldMansion] = ItemPlacementHelpers.tt1_check
            self.logic[NodeId.ValorForm][
                NodeId.SeifersTrophy
            ] = ItemPlacementHelpers.tt2_check
            self.logic[NodeId.LimitForm][
                NodeId.UndergroundConcourse
            ] = ItemPlacementHelpers.tt3_check
            self.logic[NodeId.LimitForm][
                NodeId.MansionBonus
            ] = ItemPlacementHelpers.tt3_check
        else:
            self.logic[START_NODE][
                NodeId.CentralStation
            ] = ItemPlacementHelpers.tt1_check
            self.logic[NodeId.BetwixtAndBetween][
                NodeId.SeifersTrophy
            ] = ItemPlacementHelpers.tt2_check
            self.logic[NodeId.LimitForm][
                NodeId.StationFightPopup
            ] = ItemPlacementHelpers.tt3_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    old_mansion_chests = graph.add_location(
        NodeId.OldMansionChests,
        [
    ...
    woods_chests = graph.add_location(
        NodeId.WoodsChests,
        [
    ...
    tram_common_chests = graph.add_location(
        NodeId.TramCommonChests,
        [
    ...
    station_fight_popup = graph.add_location(
        NodeId.StationFightPopup,
        [
            popup(
                526,
                CheckLocation.StationPlazaSecretAnsemReport2,
                tt,
                vanilla=report.AnsemReport2,
            ),
            popup(
                290, CheckLocation.MunnyPouchMickey, tt, vanilla=misc.MunnyPouchMickey
            ),
            popup(291, CheckLocation.CrystalOrb, tt),
        ],
    )
    central_station_chests = graph.add_location(
        NodeId.CentralStationChests,
        [
    ...
    yen_sid_tower_chests = graph.add_location(
        NodeId.YenSidTowerChests,
        [
    ...
    yen_sid_tower_entryway_chests = graph.add_location(
        NodeId.YenSidTowerEntrywayChests,
        [
    ...
    underground_concourse_chests = graph.add_location(
        NodeId.UndergroundConcourseChests,
        [
    ...
    tunnelway_chests = graph.add_location(
        NodeId.TunnelwayChests,
        [
    ...
    sunset_terrace_chests = graph.add_location(
        NodeId.SunsetTerraceChests,
        [
    ...
    mansion_bonus = graph.add_location(
        NodeId.MansionBonus,
        [
            stat_bonus(56, CheckLocation.MansionNobodies, tt),
        ],
    )
    mansion_foyer_chests = graph.add_location(
        NodeId.MansionFoyerChests,
        [
    ...
    mansion_dining_room_chests = graph.add_location(
        NodeId.MansionDiningRoomChests,
        [
    ...
    mansion_library_chests = graph.add_location(
        NodeId.MansionLibraryChests,
        [
            chest(460, CheckLocation.MansionLibraryOrichalcum, tt),
        ],
    )
    mansion_library = graph.add_location(NodeId.MansionLibrary, [])
    beam = graph.add_location(
        NodeId.TwilightTownBeam,
        [
            popup(
                534,
                CheckLocation.BeamSecretAnsemReport10,
                tt,
                vanilla=report.AnsemReport10,
            ),
        ],
    )
    mansion_basement_chests = graph.add_location(
        NodeId.MansionBasementChests,
        [
    ...
    betwixt_and_between = graph.add_location(
        NodeId.BetwixtAndBetween,
        [
            item_bonus(
                63, CheckLocation.BetwixtAndBetween, tt, vanilla=ability.Slapshot
            ),
            popup(
                317,
                CheckLocation.BetwixtAndBetweenBondOfFlame,
                tt,
                vanilla=keyblade.BondOfFlame,
            ),
        ],
    )
    data_axel = graph.add_location(
        NodeId.DataAxel,
        [
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            yen_sid_tower_entryway, sorcerers_loft, RequirementEdge(battle=True)
        )
        ...
        graph.add_edge(beam, betwixt_and_between, RequirementEdge(battle=True))
        graph.add_edge(
            betwixt_and_between, seifers_trophy, RequirementEdge(battle=True)
        )
        graph.add_edge(seifers_trophy, limit_form)
