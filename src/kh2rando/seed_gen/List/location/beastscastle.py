...
from ..inventory import magic, keyblade, ability, report
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class BCLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_bc_keyblade(inv)
        )
        self.logic[NodeId.BeastsCastleCourtyard][
            NodeId.BeastsCastleCourtyardChests
        ] = keyblade_lambda
        ...
        self.logic[NodeId.WestHallPostDungeon][
            NodeId.WestHallPostDungeonChests
        ] = keyblade_lambda
        ...
        if not reverse_rando:
            self.logic[START_NODE][
                NodeId.BeastsCastleCourtyard
            ] = ItemPlacementHelpers.bc1_check
            self.logic[NodeId.DarkThorn][
                NodeId.RumblingRose
            ] = ItemPlacementHelpers.bc2_check
        else:
            self.logic[START_NODE][NodeId.WestHall] = ItemPlacementHelpers.bc1_check
            self.logic[NodeId.Xaldin][
                NodeId.BeastsCastleCourtyard
            ] = ItemPlacementHelpers.bc2_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    belles_room_chests = graph.add_location(
        NodeId.BellesRoomChests,
        [
    ...
    east_wing_chests = graph.add_location(
        NodeId.EastWingChests,
        [
    ...
    west_hall_chests = graph.add_location(
        NodeId.WestHallChests,
        [
    ...
    thresholder = graph.add_location(
        NodeId.Thresholder,
        [
            item_bonus(2, CheckLocation.Thresholder, bc, vanilla=ability.UpperSlash),
        ],
    )
    dungeon_chests = graph.add_location(
        NodeId.DungeonChests,
        [
    ...
    secret_passage_chests = graph.add_location(
        NodeId.SecretPassageChests,
        [
    ...
    west_hall_post_dungeon_chests = graph.add_location(
        NodeId.WestHallPostDungeonChests,
        [
    ...
    west_wing_chests = graph.add_location(
        NodeId.WestWingChests,
        [
    ...
    beast_bonus = graph.add_location(
        NodeId.BeastBonus,
        [
            stat_bonus(12, CheckLocation.Beast, bc),
        ],
    )
    beasts_room_chests = graph.add_location(
        NodeId.BeastsRoomChests,
        [
            chest(241, CheckLocation.BeastsRoomBlazingShard, bc),
        ],
    )
    beasts_room = graph.add_location(NodeId.BeastsRoom, [])
    dark_thorn = graph.add_location(
        NodeId.DarkThorn,
        [
            hybrid_bonus(
                3, CheckLocation.DarkThorn, bc, vanilla=ability.RetaliatingSlash
            ),
            popup(299, CheckLocation.DarkThornCureElement, bc, vanilla=magic.Cure),
        ],
    )
    rumbling_rose = graph.add_location(
        NodeId.RumblingRose,
        [
    ...
    xaldin = graph.add_location(
        NodeId.Xaldin,
        [
            hybrid_bonus(4, CheckLocation.XaldinBonus, bc, vanilla=magic.Reflect),
            popup(
                528,
                CheckLocation.XaldinSecretAnsemReport4,
                bc,
                vanilla=report.AnsemReport4,
            ),
        ],
    )
    data_xaldin = graph.add_location(
        NodeId.DataXaldin,
        [
