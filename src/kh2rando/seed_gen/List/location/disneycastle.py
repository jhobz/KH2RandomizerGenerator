...
from ..inventory import misc, magic, keyblade, ability, form
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class DCLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_dc_keyblade(inv)
        )
        self.logic[NodeId.DisneyCastleCourtyard][
            NodeId.DisneyCastleCourtyardChests
        ] = keyblade_lambda
        self.logic[NodeId.Library][NodeId.LibraryChests] = keyblade_lambda
        self.logic[NodeId.CornerstoneHill][
            NodeId.CornerstoneHillChests
        ] = keyblade_lambda
        ...
        if not reverse_rando:
            self.logic[START_NODE][
                NodeId.DisneyCastleCourtyard
            ] = ItemPlacementHelpers.dc1_check
            self.logic[NodeId.MinnieEscort][
                NodeId.CornerstoneHill
            ] = ItemPlacementHelpers.dc2_check
        else:
            self.logic[START_NODE][
                NodeId.CornerstoneHill
            ] = ItemPlacementHelpers.dc1_check
            self.logic[NodeId.Marluxia][
                NodeId.DisneyCastleCourtyard
            ] = ItemPlacementHelpers.dc2_check

        self.logic[NodeId.WisdomPopup][
            NodeId.LingeringWill
        ] = ItemPlacementHelpers.need_proof_connection
...
def make_graph(graph: LocationGraphBuilder):
    ...
    courtyard_chests = graph.add_location(
        NodeId.DisneyCastleCourtyardChests,
        [
    ...
    library_chests = graph.add_location(
        NodeId.LibraryChests,
        [
            chest(91, CheckLocation.LibraryTornPages, dc, vanilla=misc.TornPages),
        ],
    )
    library = graph.add_location(NodeId.Library, [])
    library_popup = graph.add_location(
        NodeId.LibraryPopup,
        [
            popup(332, CheckLocation.DisneyCastleMap, dc),
        ],
    )
    minnie_escort = graph.add_location(
        NodeId.MinnieEscort,
        [
            hybrid_bonus(
                38, CheckLocation.MinnieEscort, dc, vanilla=ability.AutoSummon
            ),
        ],
    )
    cornerstone_hill_chests = graph.add_location(
        NodeId.CornerstoneHillChests,
        [
    ...
    pier_chests = graph.add_location(
        NodeId.PierChests,
        [
    ...
    waterway_chests = graph.add_location(
        NodeId.WaterwayChests,
        [
    ...
    data_marluxia = graph.add_location(
        NodeId.DataMarluxia,
        [
            popup(
                553, CheckLocation.DataMarluxiaLostIllusion, [dc, locationType.DataOrg]
            ),
        ],
    )
    lingering_will = graph.add_location(
        NodeId.LingeringWill,
        [
            stat_bonus(
                70,
                CheckLocation.LingeringWillBonus,
                [dc, locationType.LW],
                invalid_checks=[itemType.PROOF_OF_CONNECTION],
            ),
            popup(
                587,
                CheckLocation.LingeringWillProofOfConnection,
                [dc, locationType.LW],
                invalid_checks=[itemType.PROOF_OF_CONNECTION],
            ),
            popup(
                591,
                CheckLocation.LingeringWillManifestIllusion,
                [dc, locationType.LW],
                invalid_checks=[itemType.PROOF_OF_CONNECTION],
            ),
        ],
