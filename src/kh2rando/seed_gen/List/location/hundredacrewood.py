...
from ..inventory import magic, keyblade
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class HAWLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_haw_keyblade(inv)
        )
        ...
        if not reverse_rando:
            self.logic[NodeId.PoohsHowse][NodeId.PigletsHowse] = (
                ItemPlacementHelpers.need_torn_pages(1)
            )
            self.logic[NodeId.PigletsHowse][NodeId.RabbitsHowse] = (
                ItemPlacementHelpers.need_torn_pages(2)
            )
            self.logic[NodeId.RabbitsHowse][NodeId.KangasHowse] = (
                ItemPlacementHelpers.need_torn_pages(3)
            )
            self.logic[NodeId.KangasHowse][NodeId.SpookyCave] = (
                ItemPlacementHelpers.need_torn_pages(4)
            )
            self.logic[NodeId.SpookyCave][NodeId.StarryHill] = (
                ItemPlacementHelpers.need_torn_pages(5)
            )
        else:
            self.logic[START_NODE][
                NodeId.StarryHill
            ] = keyblade_lambda  # need to do this because pages can lock themselves in Reverse Rando
            self.logic[NodeId.StarryHill][NodeId.SpookyCave] = (
                ItemPlacementHelpers.need_torn_pages(1)
            )
            self.logic[NodeId.SpookyCave][NodeId.KangasHowse] = (
                ItemPlacementHelpers.need_torn_pages(2)
            )
            self.logic[NodeId.KangasHowse][NodeId.RabbitsHowse] = (
                ItemPlacementHelpers.need_torn_pages(3)
            )
            self.logic[NodeId.RabbitsHowse][NodeId.PigletsHowse] = (
                ItemPlacementHelpers.need_torn_pages(4)
            )
            self.logic[NodeId.PigletsHowse][NodeId.PoohsHowse] = (
                ItemPlacementHelpers.need_torn_pages(5)
            )
...
def make_graph(graph: LocationGraphBuilder):
    ...
    poohs_howse_chests = graph.add_location(
        NodeId.PoohsHowseChests,
        [
    ...
    piglets_howse_chests = graph.add_location(
        NodeId.PigletsHowseChests,
        [
    ...
    rabbits_howse_chests = graph.add_location(
        NodeId.RabbitsHowseChests,
        [
    ...
    kangas_howse_chests = graph.add_location(
        NodeId.KangasHowseChests,
        [
    ...
    spooky_cave_chests = graph.add_location(
        NodeId.SpookyCaveChests,
        [
    ...
    spooky_cave = graph.add_location(
        NodeId.SpookyCave,
        [
            popup(
                284, CheckLocation.SweetMemories, haw, vanilla=keyblade.SweetMemories
            ),
            popup(485, CheckLocation.SpookyCaveMap, haw),
        ],
    )
    starry_hill_chests = graph.add_location(
        NodeId.StarryHillChests,
        [
    ...
    starry_hill_popups = graph.add_location(
        NodeId.StarryHillPopups,
        [
