...
from ..inventory import magic, keyblade, misc
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class PLLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_pl_keyblade(inv)
        )
        self.logic[NodeId.Gorge][NodeId.GorgeChests] = keyblade_lambda
        self.logic[NodeId.ElephantGraveyard][
            NodeId.ElephantGraveyardChests
        ] = keyblade_lambda
        self.logic[NodeId.PrideRock][NodeId.PrideRockChests] = keyblade_lambda
        self.logic[NodeId.WildebeestValley][
            NodeId.WildebeestValleyChests
        ] = keyblade_lambda
        ...
        if not reverse_rando:
            ...
            self.logic[NodeId.Scar][
                NodeId.Hyenas2Bonus
            ] = ItemPlacementHelpers.pl2_check
        else:
            self.logic[START_NODE][NodeId.PrideRock] = ItemPlacementHelpers.pl1_check
            self.logic[NodeId.Groundshaker][
                NodeId.Gorge
            ] = ItemPlacementHelpers.pl2_check
...
def make_graph(graph: LocationGraphBuilder):
    pl = locationType.PL
    pl_logic = PLLogicGraph(graph.reverse_rando, graph.keyblades_unlock_chests)
    graph.add_logic(pl_logic)

    gorge_chests = graph.add_location(
        NodeId.GorgeChests,
        [
    ...
    elephant_graveyard_chests = graph.add_location(
        NodeId.ElephantGraveyardChests,
        [
    ...
    pride_rock_chests = graph.add_location(
        NodeId.PrideRockChests,
        [
    ...
    wildebeest_valley_chests = graph.add_location(
        NodeId.WildebeestValleyChests,
        [
    ...
    wastelands_chests = graph.add_location(
        NodeId.WastelandsChests,
        [
    ...
    jungle_chests = graph.add_location(
        NodeId.JungleChests,
        [
    ...
    oasis_chests = graph.add_location(
        NodeId.OasisChests,
        [
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            scar,
            hyenas_2_bonus,
            RequirementEdge(battle=True, req=ItemPlacementHelpers.pl2_check),
        )
        ...
        graph.add_edge(
            groundshaker, gorge, RequirementEdge(req=ItemPlacementHelpers.pl2_check)
        )
