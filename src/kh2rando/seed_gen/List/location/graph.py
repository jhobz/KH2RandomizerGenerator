...
from ..configDict import locationType, itemType, locationCategory
...
class RequirementEdge:
    ...
    def __init__(
        self, req: Optional[RequirementFunction] = None, strict=True, battle=False
    ):
...
class LocationGraphBuilder:
        ...
        for location in locations:
            ...
            if location_name in self.pending_locations_by_name:
                raise GeneratorException(
                    f"Location {location_name} is already in the graph"
                )
            else:
    ...
    def add_edge(
        self, node1: str, node2: str, requirement: Optional[RequirementEdge] = None
    ):
...
def _make_location(
    ...
    return KH2Location(
        loc_id, description, category, loc_types, invalid_checks, vanilla_items
    )
...
def chest(
    ...
    return _make_location(
        loc_id, description, locationCategory.CHEST, loc_types, invalid_checks, vanilla
    )
...
def popup(
    ...
    return _make_location(
        loc_id, description, locationCategory.POPUP, loc_types, invalid_checks, vanilla
    )
