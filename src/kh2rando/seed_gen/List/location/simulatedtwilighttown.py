...
from ..inventory import ability, misc
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class STTLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_stt_keyblade(inv)
        )
        self.logic[NodeId.RoxasStation][NodeId.RoxasStationChests] = keyblade_lambda
        self.logic[NodeId.SimulatedCentralStation][
            NodeId.SimulatedCentralStationChests
        ] = keyblade_lambda
        self.logic[NodeId.SimulatedSunsetTerrace][
            NodeId.SimulatedSunsetTerraceChests
        ] = keyblade_lambda
        self.logic[NodeId.SimulatedMansionFoyer][
            NodeId.SimulatedMansionFoyerChests
        ] = keyblade_lambda
        self.logic[NodeId.SimulatedMansionDiningRoom][
            NodeId.SimulatedMansionDiningRoomChests
        ] = keyblade_lambda
        self.logic[NodeId.SimulatedMansionLibrary][
            NodeId.SimulatedMansionLibraryChests
        ] = keyblade_lambda
        self.logic[NodeId.SimulatedMansionBasement][
            NodeId.SimulatedMansionBasementChests
        ] = keyblade_lambda
        if not reverse_rando:
            self.logic[START_NODE][
                NodeId.TwilightTownMapPopup
            ] = ItemPlacementHelpers.stt_check
        else:
            self.logic[START_NODE][
                NodeId.SimulatedMansionFoyer
            ] = ItemPlacementHelpers.stt_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    twilight_town_map_popup = graph.add_location(
        NodeId.TwilightTownMapPopup,
        [
            popup(319, CheckLocation.TwilightTownMap, stt),
        ],
    )
    munny_pouch_popup = graph.add_location(
        NodeId.MunnyPouchPopup,
        [
            popup(
                288, CheckLocation.MunnyPouchOlette, stt, vanilla=misc.MunnyPouchOlette
            ),
        ],
    )
    roxas_station_chests = graph.add_location(
        NodeId.RoxasStationChests,
        [
            chest(315, CheckLocation.StationOfSerenityPotion, stt),
            chest(472, CheckLocation.StationOfCallingPotion, stt),
        ],
    )
    roxas_station = graph.add_location(
        NodeId.RoxasStation,
        [
            item_bonus(
                54, CheckLocation.StationDusks, stt, vanilla=ability.AerialRecovery
            ),
        ],
    )
    twilight_thorn = graph.add_location(
        NodeId.TwilightThorn,
        [
            item_bonus(33, CheckLocation.TwilightThorn, stt, vanilla=ability.Guard),
        ],
    )
    axel_1 = graph.add_location(
        NodeId.Axel1,
        [
            item_bonus(73, CheckLocation.Axel1, stt, vanilla=ability.Scan),
        ],
    )
    struggle_champion = graph.add_location(
        NodeId.StruggleChampion,
        [
    ...
    namines_room = graph.add_location(
        NodeId.NaminesRoom,
        [
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(roxas_station, twilight_thorn, RequirementEdge(battle=True))
        graph.add_edge(
            twilight_thorn, twilight_town_map_popup, RequirementEdge(battle=True)
        )
        graph.add_edge(twilight_town_map_popup, munny_pouch_popup)
