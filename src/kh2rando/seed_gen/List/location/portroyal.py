...
from ..inventory import magic, keyblade, ability, misc, summon, report
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class PRLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_pr_keyblade(inv)
        )
        ...
        self.logic[NodeId.InterceptorsHold][
            NodeId.InterceptorsHoldChests
        ] = keyblade_lambda
        ...
        if not reverse_rando:
            ...
            self.logic[NodeId.Barbossa][
                NodeId.GrimReaper1
            ] = ItemPlacementHelpers.pr2_check
        else:
            self.logic[START_NODE][NodeId.Rampart] = ItemPlacementHelpers.pr1_check
            self.logic[NodeId.GrimReaper2][
                NodeId.PortRoyalTown
            ] = ItemPlacementHelpers.pr2_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    rampart_chests = graph.add_location(
        NodeId.RampartChests,
        [
    ...
    town_chests = graph.add_location(
        NodeId.PortRoyalTownChests,
        [
    ...
    cave_mouth_chests = graph.add_location(
        NodeId.CaveMouthChests,
        [
    ...
    isla_de_muerta_popup = graph.add_location(
        NodeId.IslaDeMuertaPopup,
        [
            popup(329, CheckLocation.IslaDeMuertaMap, pr),
        ],
    )
    boat_fight = graph.add_location(
        NodeId.BoatFight,
        [
            item_bonus(62, CheckLocation.BoatFight, pr, vanilla=ability.AerialSpiral),
        ],
    )
    barrels_minigame = graph.add_location(
        NodeId.BarrelsMinigame,
        [
            stat_bonus(39, CheckLocation.InterceptorBarrels, pr),
        ],
    )
    powder_store_chests = graph.add_location(
        NodeId.PowderStoreChests,
        [
    ...
    moonlight_nook_chests = graph.add_location(
        NodeId.MoonlightNookChests,
        [
    ...
    grim_reaper_1 = graph.add_location(
        NodeId.GrimReaper1,
        [
            item_bonus(
                59, CheckLocation.GrimReaper1, pr, vanilla=ability.HorizontalSlash
            ),
        ],
    )
    interceptors_hold_chests = graph.add_location(
        NodeId.InterceptorsHoldChests,
        [
            chest(
                252,
                CheckLocation.InterceptorsHoldFeatherCharm,
                pr,
                vanilla=summon.FeatherCharm,
            ),
        ],
    )
    interceptors_hold = graph.add_location(NodeId.InterceptorsHold, [])
    seadrift_keep_chests = graph.add_location(
        NodeId.SeadriftKeepChests,
        [
    ...
    seadrift_row_chests = graph.add_location(
        NodeId.SeadriftRowChests,
        [
    ...
    cursed_medallion_popup = graph.add_location(
        NodeId.CursedMedallionPopup,
        [
            popup(
                296,
                CheckLocation.SeadriftRowCursedMedallion,
                pr,
                vanilla=misc.CursedMedallion,
            ),
            popup(331, CheckLocation.SeadriftRowShipGraveyardMap, pr),
        ],
    )
    grim_reaper_2 = graph.add_location(
        NodeId.GrimReaper2,
        [
            item_bonus(22, CheckLocation.GrimReaper2, pr, vanilla=magic.Magnet),
            popup(
                530,
                CheckLocation.GrimReaper2SecretAnsemReport6,
                pr,
                vanilla=report.AnsemReport6,
            ),
        ],
    )
    data_luxord = graph.add_location(
        NodeId.DataLuxord,
        [
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            seadrift_keep, cursed_medallion_popup, RequirementEdge(battle=True)
        )
        graph.add_edge(
            cursed_medallion_popup, grim_reaper_2, RequirementEdge(battle=True)
        )
