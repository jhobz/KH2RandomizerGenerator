...
from ..inventory import misc, magic, keyblade, ability, summon
from .graph import (
    RequirementEdge,
    chest,
    popup,
...
class AGLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_ag_keyblade(inv)
        )
        ...
        self.logic[NodeId.CaveOfWondersEntrance][
            NodeId.CaveOfWondersEntranceChests
        ] = keyblade_lambda
        self.logic[NodeId.ValleyOfStone][NodeId.ValleyOfStoneChests] = keyblade_lambda
        self.logic[NodeId.ChasmOfChallenges][
            NodeId.ChasmOfChallengesChests
        ] = keyblade_lambda
        ...
        if not reverse_rando:
            self.logic[START_NODE][
                NodeId.AgrabahMapPopup
            ] = ItemPlacementHelpers.ag1_check
            self.logic[NodeId.ElementalLords][NodeId.RuinedChamber] = (
                lambda inv: ItemPlacementHelpers.ag2_check(inv)
            ...
            self.logic[NodeId.PalaceWalls][
                NodeId.RuinedChamber
            ] = ItemPlacementHelpers.need_fire_blizzard_thunder
            self.logic[NodeId.Lexaeus][
                NodeId.AgrabahMapPopup
            ] = ItemPlacementHelpers.ag2_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    agrabah_map_popup = graph.add_location(
        NodeId.AgrabahMapPopup,
        [
            popup(353, CheckLocation.AgrabahMap, ag),
        ],
    )
    agrabah_chests = graph.add_location(
        NodeId.AgrabahChests,
        [
    ...
    bazaar_chests = graph.add_location(
        NodeId.BazaarChests,
        [
    ...
    palace_walls_chests = graph.add_location(
        NodeId.PalaceWallsChests,
        [
    ...
    cave_of_wonders_entrance_chests = graph.add_location(
        NodeId.CaveOfWondersEntranceChests,
        [
    ...
    valley_of_stone_chests = graph.add_location(
        NodeId.ValleyOfStoneChests,
        [
    ...
    abu_escort = graph.add_location(
        NodeId.AbuEscort,
        [
            item_bonus(42, CheckLocation.AbuEscort, ag, vanilla=ability.SummonBoost),
        ],
    )
    chasm_of_challenges_chests = graph.add_location(
        NodeId.ChasmOfChallengesChests,
        [
    ...
    treasure_room_bonus = graph.add_location(
        NodeId.TreasureRoomBonus,
        [
            stat_bonus(46, CheckLocation.TreasureRoomBonus, ag),
        ],
    )
    treasure_room_chests = graph.add_location(
        NodeId.TreasureRoomChests,
        [
    ...
    elemental_lords = graph.add_location(
        NodeId.ElementalLords,
        [
            item_bonus(
                37, CheckLocation.ElementalLordsBonus, ag, vanilla=ability.FinishingLeap
            ),
            popup(300, CheckLocation.LampCharm, ag, vanilla=summon.LampCharm),
        ],
    )
    ruined_chamber_chests = graph.add_location(
        NodeId.RuinedChamberChests,
        [
    ...
    genie_jafar = graph.add_location(
        NodeId.GenieJafar,
        [
    ...
    lexaeus = graph.add_location(
        NodeId.Lexaeus,
        [
            stat_bonus(65, CheckLocation.LexaeusBonus, [ag, locationType.AS]),
            popup(
                545, CheckLocation.LexaeusStrengthBeyondStrength, [ag, locationType.AS]
            ),
        ],
    )
    data_lexaeus = graph.add_location(
        NodeId.DataLexaeus,
        [
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            chasm_of_challenges, treasure_room_bonus, RequirementEdge(battle=True)
        )
