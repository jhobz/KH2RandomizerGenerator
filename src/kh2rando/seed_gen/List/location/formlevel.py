...
from ..inventory import ability, form as driveform, growth
from ..inventory.form import DriveForm
from ..inventory.item import InventoryItem
from .graph import RequirementEdge, LocationGraphBuilder, START_NODE
...
def form_level(
    level: int, form: DriveForm, vanilla: Optional[InventoryItem] = None
) -> KH2Location:
    ...
    return KH2Location(
        level, description, form.level_category, types, VanillaItems=vanilla_items
    )
...
def make_graph(graph: LocationGraphBuilder):
    ...
    for form, vanilla_rewards in forms_with_vanilla_rewards.items():
        ...
        for level in range(1, 8):
            locations[level] = graph.add_location(
                f"{form.short_name}-{level}",
        ...
        for level in range(2, 8):
            graph.add_edge(
                locations[level - 1],
                locations[level],
                RequirementEdge(req=form_helper(form, level)),
            )
