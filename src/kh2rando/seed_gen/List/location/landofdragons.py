...
from ..inventory import misc, magic, keyblade, ability
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class LoDLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_lod_keyblade(inv)
        )
        ...
        if not reverse_rando:
            ...
            self.logic[NodeId.ShanYu][
                NodeId.ThroneRoom
            ] = ItemPlacementHelpers.lod2_check
        else:
            self.logic[START_NODE][
                NodeId.MountainTrail
            ] = ItemPlacementHelpers.lod1_check
            self.logic[NodeId.StormRider][
                NodeId.BambooGrove
            ] = ItemPlacementHelpers.lod2_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    bamboo_grove_chests = graph.add_location(
        NodeId.BambooGroveChests,
        [
    ...
    encampment_area_map = graph.add_location(
        NodeId.EncampmentAreaMap,
        [
            popup(350, CheckLocation.EncampmentAreaMap, lod),
        ],
    )
    mission3 = graph.add_location(
        NodeId.Mission3,
        [
            popup(417, CheckLocation.Mission3, lod),
        ],
    )
    checkpoint_chests = graph.add_location(
        NodeId.CheckpointChests,
        [
    ...
    mountain_trail_chests = graph.add_location(
        NodeId.MountainTrailChests,
        [
    ...
    village_cave_map_popup = graph.add_location(
        NodeId.VillageCaveMapPopup,
        [
            popup(495, CheckLocation.VillageCaveAreaMap, lod),
        ],
    )
    village_cave_chests = graph.add_location(
        NodeId.VillageCaveChests,
        [
    ...
    village_cave_bonus = graph.add_location(
        NodeId.VillageCaveBonus,
        [
            item_bonus(
                43, CheckLocation.VillageCaveBonus, lod, vanilla=ability.SlideDash
            ),
        ],
    )
    ridge_chests = graph.add_location(
        NodeId.RidgeChests,
        [
    ...
    shan_yu = graph.add_location(
        NodeId.ShanYu,
        [
            hybrid_bonus(
                9, CheckLocation.ShanYuBonus, lod, vanilla=ability.AerialSweep
            ),
            popup(257, CheckLocation.HiddenDragon, lod, vanilla=keyblade.HiddenDragon),
        ],
    )
    throne_room_chests = graph.add_location(
        NodeId.ThroneRoomChests,
        [
    ...
    storm_rider = graph.add_location(
        NodeId.StormRider,
        [
            item_bonus(10, CheckLocation.StormRiderBonus, lod, vanilla=magic.Thunder),
        ],
    )
    data_xigbar = graph.add_location(
        NodeId.DataXigbar,
        [
            popup(
                555, CheckLocation.DataXigbarDefenseBoost, [lod, locationType.DataOrg]
            ),
        ],
    )
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            mountain_trail, village_cave_map_popup, RequirementEdge(battle=True)
        )
        ...
        graph.add_edge(
            shan_yu,
            throne_room,
            RequirementEdge(battle=True, req=ItemPlacementHelpers.lod2_check),
        )
        ...
        graph.add_edge(
            storm_rider,
            bamboo_grove,
            RequirementEdge(req=ItemPlacementHelpers.lod2_check),
        )
