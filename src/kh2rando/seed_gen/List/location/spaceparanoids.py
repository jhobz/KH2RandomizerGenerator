...
from ..inventory import ability, keyblade, magic
from .graph import (
    DefaultLogicGraph,
    RequirementEdge,
    chest,
...
class SPLogicGraph(DefaultLogicGraph):
    def __init__(self, reverse_rando, keyblade_unlocks):
        ...
        keyblade_lambda = (
            ...
            or ItemPlacementHelpers.need_sp_keyblade(inv)
        )
        ...
        self.logic[NodeId.CommunicationsRoom][
            NodeId.CommunicationsRoomChests
        ] = keyblade_lambda
        self.logic[NodeId.CentralComputerCore][
            NodeId.CentralComputerCoreChests
        ] = keyblade_lambda
        if not reverse_rando:
            self.logic[START_NODE][NodeId.PitCell] = ItemPlacementHelpers.sp1_check
            self.logic[NodeId.PhotonDebugger][
                NodeId.SolarSailerBonus
            ] = ItemPlacementHelpers.sp2_check
        else:
            self.logic[START_NODE][NodeId.PitCell] = ItemPlacementHelpers.sp1_check
            self.logic[NodeId.Larxene][
                NodeId.ScreensBonus
            ] = ItemPlacementHelpers.sp2_check
...
def make_graph(graph: LocationGraphBuilder):
    ...
    pit_cell_chests = graph.add_location(
        NodeId.PitCellChests,
        [
    ...
    canyon_chests = graph.add_location(
        NodeId.CanyonChests,
        [
    ...
    screens_bonus = graph.add_location(
        NodeId.ScreensBonus,
        [
            stat_bonus(45, CheckLocation.ScreensBonus, sp),
        ],
    )
    hallway_chests = graph.add_location(
        NodeId.HallwayChests,
        [
    ...
    communications_room_chests = graph.add_location(
        NodeId.CommunicationsRoomChests,
        [
    ...
    hostile_program = graph.add_location(
        NodeId.HostileProgram,
        [
            hybrid_bonus(
                31, CheckLocation.HostileProgramBonus, sp, vanilla=ability.VicinityBreak
            ),
        ],
    )
    photon_debugger = graph.add_location(
        NodeId.PhotonDebugger,
        [
            popup(
                267, CheckLocation.PhotonDebugger, sp, vanilla=keyblade.PhotonDebugger
            ),
        ],
    )
    solar_sailer_bonus = graph.add_location(
        NodeId.SolarSailerBonus,
        [
            item_bonus(
                61, CheckLocation.SolarSailerBonus, sp, vanilla=ability.Explosion
            ),
        ],
    )
    central_computer_core_chests = graph.add_location(
        NodeId.CentralComputerCoreChests,
        [
    ...
    mcp_bonus = graph.add_location(
        NodeId.MasterControlProgramBonus,
        [
            hybrid_bonus(32, CheckLocation.McpBonus, sp, vanilla=magic.Reflect),
        ],
    )
    larxene = graph.add_location(
        NodeId.Larxene,
        [
    ...
    data_larxene = graph.add_location(
        NodeId.DataLarxene,
        [
            popup(
                552, CheckLocation.DataLarxeneLostIllusion, [sp, locationType.DataOrg]
            ),
        ],
    )
    ...
    if not graph.reverse_rando:
        ...
        graph.add_edge(
            communications_room, hostile_program, RequirementEdge(battle=True)
        )
