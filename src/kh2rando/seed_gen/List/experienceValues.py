...
from .configDict import expCurve
...
def create_new_values(
    second_deriv=20, first_deriv=40, early_offset=0, mid_offset=0, late_offset=0
) -> list[int]:
...
def get_form_exp(drive_form: DriveForm, rate: float, curve: expCurve) -> list[int]:
    if curve == expCurve.DAWN:
        ...
        raise SettingsException(f"Incorrect exp curve value {curve}")
    return [
        math.ceil(a / b) for a, b in zip((exp_list[i] for i in range(1, 8)), [rate] * 7)
    ]
