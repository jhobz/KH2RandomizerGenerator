...
from .configDict import locationCategory
...
def get_all_parent_edge_requirements(node_id: str, graph: Graph) -> RequirementFunction:
    ...
    for edge in graph.inc_edges(node_id):
        ...
        if data.requirement is not None:
            requirement = lambda inv, this_req=data.requirement, par_req=parent_requirements: this_req(
                inv
            ) and par_req(
                inv
            )
    ...
    if len(non_strict_requirements) > 0:
        ...
        new_strict_func = lambda inv, non_s_req=non_strict_requirements: any(
            [r(inv) for r in non_s_req]
        )
        strict_requirements.append(new_strict_func)
    this_requirement = lambda inv, s_req=strict_requirements: all(
        [r(inv) for r in s_req]
    )
...
class Locations:
    ...
    def node_ids_before(
        self, node_id: str, include_self: bool = False, include_starting: bool = False
    ) -> list[str]:
        """
    ...
    def locations_for_category(self, category: locationCategory) -> list[KH2Location]:
        ...
        return [
            location
            for location in self._all_locations_iter()
            if category == location.LocationCategory
        ]
    ...
    def locations_before(
        ...
        """
        result = []
        for before_node_id in self.node_ids_before(
            node_id, include_self, include_starting_node
        ):
    ...
    def all_donald_locations() -> list[KH2Location]:
        return (
            donaldbonus.donald_bonuses()
            + weaponslot.donald_staff_slots()
            + starting.donald_starting_items()
        )

    @staticmethod
    def all_goofy_locations() -> list[KH2Location]:
        return (
            goofybonus.goofy_bonuses()
            + weaponslot.goofy_shield_slots()
            + starting.goofy_starting_items()
        )

    def make_location_graph(self, settings: RandomizerSettings):
        builder = LocationGraphBuilder(
            self.location_graph, self.reverse_rando, settings
        )
