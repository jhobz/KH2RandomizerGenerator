...
from .itemPlacementRestriction import ItemPlacementHelpers
from .newRandomize import Randomizer, SynthesisRecipe
...
class LocationInformedSeedValidator:
    ...
    def populate_possible_locking_item_list(
        self, regular_rando: bool, keyblade_locking: bool
    ):
        # STT
        stt_graph = Graph()
        ...
        hb_graph.add_node(
            "HB2", [storyunlock.MembershipCard, storyunlock.MembershipCard]
        )
        ...
        oc_graph.add_node(
            "OC2", [storyunlock.BattlefieldsOfWar, storyunlock.BattlefieldsOfWar]
        )
        ...
        lod_graph.add_node(
            "LoD2", [storyunlock.SwordOfTheAncestor, storyunlock.SwordOfTheAncestor]
        )
        ...
        tt_graph.add_node(
            "TT3", [storyunlock.IceCream, storyunlock.IceCream, storyunlock.IceCream]
        )
        ...
        haw_graph.add_node(
            "Page4", [misc.TornPages, misc.TornPages, misc.TornPages, misc.TornPages]
        )
        haw_graph.add_node(
            "Page5",
            [
        ...
        pr_graph.add_node(
            "PR2", [storyunlock.SkillAndCrossbones, storyunlock.SkillAndCrossbones]
        )
        ...
        twtnw_graph.add_node(
            "TWTNW2", [storyunlock.WayToTheDawn, storyunlock.WayToTheDawn]
        )
        ...
        atlantica_graph.add_node(
            "Song5",
            [magic.Magnet, magic.Magnet, magic.Thunder, magic.Thunder, magic.Thunder],
        )
        ...
        if regular_rando:
            ...
            ag_graph = Graph()
            ag_graph.add_node(
                "AG1", [storyunlock.Scimitar]
            )  # there are no first visit checks in reverse without the key
            ...
            if keyblade_locking:
                ag_graph.add_node(
                    "AGChests", [storyunlock.Scimitar, keyblade.WishingLamp]
                )
                ag_graph.add_edge("AG1", "AGChests")
    ...
    def generate_locking_item_ids(self):
        def random_walk(graph: Graph):
            ...
            current_item_data = []
            current_node = next(
                (n for n in graph.node_list() if graph.inc_degree(n) == 0), None
            )
            ...
            while True:
                ...
                candidate_nodes = [
                    n
                    for n in graph.out_nbrs(current_node)
                    if n not in current_node_list
                ]
    ...
    def is_location_available(
        self, inventory: list[int], location: KH2Location
    ) -> bool:
        return self.evaluate(inventory, self.location_requirements[location])

    def prepare_requirements_list(
        self, location_lists: list[Locations], synthesis_recipes: list[SynthesisRecipe]
    ):
        self.location_requirements.clear()
        for locations in location_lists:
            for node_id in locations.node_ids():
                parent_requirement_function = get_all_parent_edge_requirements(
                    node_id, locations.location_graph
                )
                for location in locations.locations_for_node(node_id):
                    ...
                    self.location_requirements[location].append(
                        parent_requirement_function
                    )
            ...
            if locationType.SYNTH in loc.LocationTypes:
                ...
                if recipe:
                    for recipe_requirement in recipe.requirements:
                        ...
                        requirements.append(
                            ItemPlacementHelpers.make_synth_requirement(synth_item)
                        )

    def prep_requirements_list(
        self, settings: RandomizerSettings, randomizer: Randomizer
    ):
        location_lists = []
        ...
        self.populate_possible_locking_item_list(
            settings.regular_rando, settings.keyblades_unlock_chests
        )

    def validate_seed(
        self, settings: RandomizerSettings, randomizer: Randomizer, verbose: bool = True
    ) -> list[KH2Location]:
        ...
        if (
            settings.item_accessibility == ItemAccessibilityOption.ALL
            and results.full_clear
        ) or (
            settings.item_accessibility == ItemAccessibilityOption.BEATABLE
            and results.any_percent
        ):
            # we all good
            if verbose:
            ...
            raise ValidationException(
                f"Completion checking failed to collect {len(location_requirements)} items"
            )
