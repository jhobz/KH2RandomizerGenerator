...
from .newRandomize import Randomizer, ItemAssignment
...
class MultiWorldEntry:
    ...
    def __call__(self):
        location_category = ""
        if self.where_found.LocationCategory in [
        ...
        return {
            "name": self.what_found.Id,
            "location": location_category + "-" + str(self.where_found.LocationId),
            "to": self.owner,
            "from": self.finder,
        }
...
class MultiWorldOutput:
    ...
    def add_item(self, item, location, from_player, to_player):
        self.new_assignments.append(
            MultiWorldEntry(from_player, to_player, location, item)
        )
    ...
    def __init__(self, seeds: List[Randomizer], config: MultiWorldConfig):
        ...
        for seed in seeds:
            ...
            for assignment in seed.assignments:
                if assignment.location.LocationCategory in config.location_swaps:
                    if (
                        assignment.item is not None
                        and assignment.item.ItemType in config.item_swaps
                    ):
        ...
        for player in range(len(self.all_candidate_swaps)):
            ...
            for assignment in candiate_swaps:
                print(
                    f"--- ({assignment.item},{assignment.item2}) in {assignment.location.Description}({assignment.location.LocationId})"
                )
