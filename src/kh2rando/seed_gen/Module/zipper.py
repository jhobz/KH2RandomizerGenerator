...
from .battleLevels import BtlvViewer
from .cosmetics import CosmeticsMod
from .hints import Hints, HintData
from .knockbackTypes import KnockbackTypes
from .multiworld import MultiWorldOutput
from .newRandomize import Randomizer, SynthesisRecipe, ItemAssignment
from .resources import resource_path
from .seedmod import SeedModBuilder, ChestVisualAssignment, CosmeticsModAppender
from .spoilerLog import (
...
from .version import LOCAL_UI_VERSION
...
def _invoke_khbr_with_overrides(
    ...
    enemySpoilers = BossEnemyRandomizer().generateToZip(
        "kh2", enemy_options, mod, out_zip
    )
...
class SeedZip:
        ...
        with ZipFile(zip_data, "w", ZIP_DEFLATED) as out_zip:
            ...
            if settings.dummy_forms:
                ...
                for orig_id, dummy_id in orig_to_dummy.items():
                    self.randomizer.starting_item_ids[:] = [
                        dummy_id if x == orig_id else x
                        for x in self.randomizer.starting_item_ids
                    ]
            ...
            if settings.objective_rando:
                self.create_objective_rando_assets(
                    mod, settings.num_objectives_needed, self.randomizer.objectives
                )
            if settings.emblems:
            ...
            _add_cosmetics(
                out_zip=out_zip, mod_yml=mod.mod_yml, settings=settings.ui_settings
            )
    ...
    def create_objective_rando_assets(
        self,
        mod: SeedModBuilder,
        num_objectives_needed: int,
        objective_list: list[KH2Objective],
    ):
    ...
    def assign_starting_items(self, mod: SeedModBuilder):
        ...
        growth_abilities = [
            i.Id for i in master_item_list if (i.ItemType == itemType.GROWTH_ABILITY)
        ]
        sora_abilities = [
            i.Id
        ...
        donald_starting_items = [1 + 0x8000, 3 + 0x8000] + [
            l.item.Id
        ...
        sora_bdscript_items = [
            i for i in randomizer.starting_item_ids if i not in sora_abilities
        ]

        mod.add_sora_bdscript(
            sora_bdscript_items, sora_bdscript_abilities, settings.auto_equip_abilities
        )
                ...
                for lvl in levels:
                    if lvl.location.LocationId == level_number:
                        ...
                        mod.form_levels.add_form_level(
                            ...
                            ability=(
                                lvl.item.Id if index != 0 else 0
                            ),  # making summon junk items zero
    ...
    def _final_door_requirement_message(self) -> str:
        ...
        if (
            settings.promiseCharm
            or misc.PromiseCharm.id in settings.starting_inventory_ids
        ):
        ...
        if settings.objective_rando:
            message_lines.append(
                f"{settings.num_objectives_needed} completed objectives."
            )
    ...
    def check_if_companion_changed(self, settings: SeedSettings, keys: settingkey):
    ...
    def prepare_companion_damage_knockback(self, mod: SeedModBuilder):
        ...
        options_list.append(
            knockback_organizer(ui_settings.get(keys.DONALD_BLIZZARD_KNOCKBACK_TYPE))
        )
        options_list.append(
        ...
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SKELLINGTON_BLAZING_FURY_KNOCKBACK_TYPE)
            )
        )
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SKELLINGTON_ICY_TERROR_KNOCKBACK_TYPE)
            )
        )
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SKELLINGTON_BOLTS_OF_SORROW_KNOCKBACK_TYPE)
            )
        )
        self.ready_companion_damage_knockback_atkp_entries(
        ...
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.ALADDIN_SLASH_FRENZY_KNOCKBACK_TYPE)
            )
        )
        options_list.append(
        ...
        options_list = []
        options_list.append(
            knockback_organizer(ui_settings.get(keys.TRON_MELEE_ATTACKS_KNOCKBACK_TYPE))
        )
        options_list.append(
        ...
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.TRON_PULSING_THUNDER_KNOCKBACK_TYPE)
            )
        )
        self.ready_companion_damage_knockback_atkp_entries(
        ...
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SPARROW_NO_MERCY_KNOCKBACK_TYPE)
            )
        )
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SPARROW_RAIN_STORM_KNOCKBACK_TYPE)
            )
        )
        options_list.append(
            knockback_organizer(
                ui_settings.get(keys.JACK_SPARROW_BONE_SMASH_KNOCKBACK_TYPE)
            )
        )
        self.ready_companion_damage_knockback_atkp_entries(
        ...
        options_list = []
        options_list.append(
            knockback_organizer(ui_settings.get(keys.RIKU_MELEE_ATTACKS_KNOCKBACK_TYPE))
        )
        options_list.append(
        ...
        for melee_entry in companion_melee_ids:
            if len(melee_entry) == 3:
                companion_melee_objects.append(
                    atkp_organizer.get_attack_using_ids_plus_switch(
                        melee_entry[0], melee_entry[1], melee_entry[2]
                    )
                )
            companion_melee_objects.append(
        ...
        for i in range(len(companion_ability_ids)):
            ...
            for ability_entry in companion_ability_ids[i]:
                if len(ability_entry) == 3:
                    ...
                    entry_object.EnemyReaction = melee_ability_knockback_types_list[
                        i + 1
                    ]
