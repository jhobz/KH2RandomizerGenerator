...
from .newRandomize import ItemAssignment, SynthesisRecipe, WeaponStats
...
def item_spoiler_dictionary(
    ...
    if starting_inventory_ids is not None:
        ...
        for index, starting_item_id in enumerate(starting_inventory_ids):
            ...
            if item is not None:
                ...
                print(
                    f"Could not find item with id {starting_item_id} to add to the spoiler log"
                )
    ...
    for assignment in item_assignments:
        ...
        if len(location.LocationTypes) > 0:
            ...
            if assignment.item.item != misc.NullItem:
                out_dict[location_type].append(
                    (sort_depth, loc_id, sphere, spoiler_string, assignment.item)
                )
            if assignment.item2 is not None:
                out_dict[location_type].append(
                    (sort_depth, loc_id, sphere, spoiler_string, assignment.item2)
                )
    ...
    for location_type, entries in out_dict.items():
        ...
        result_dict[location_type] = [
            (entry[2], entry[3], entry[4]) for entry in entries
        ]
...
def synth_recipe_dictionary(
    assignments: list[ItemAssignment], recipes: list[SynthesisRecipe]
) -> list[dict[str, str]]:
    ...
    for recipe in sorted_recipes:
        ...
        for requirement in recipe.requirements:
            requirement_strings.append(
                " x".join([requirement.synth_item.Name, f"{requirement.amount}"])
            )
        result.append(
            {
