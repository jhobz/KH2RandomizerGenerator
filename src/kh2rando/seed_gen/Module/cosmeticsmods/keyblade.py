...
def _vanilla_keyblades() -> list[VanillaKeyblade]:
    return [
        ...
        VanillaKeyblade(
            "Oathkeeper", "044", "se501", "W_EX010_10", can_replace_effects=False
        ),
        ...
        VanillaKeyblade(
            "Sleeping Lion", "058", "se519", "W_EX010_F0", can_replace_effects=False
        ),
...
class KeybladeRandomizer:
    ...
    def extract_keyblade(
        ...
        if openkh_path is None:
            raise GeneratorException(
                "No OpenKH path configured. Can't extract keyblades."
            )

        bar_exe = openkh_path / "OpenKh.Command.Bar.exe"
        if not bar_exe.is_file():
            raise GeneratorException(
                "No OpenKh.Command.Bar.exe found. Can't extract keyblades."
            )
        ...
        for index, model_type in enumerate(["base", "nm", "tr", "wi"]):
            ...
            if remastered_mdlx_folder.is_dir():
                copy_remastered_textures(
                    remastered_mdlx_folder, out_path / "remastered-textures"
                )
            ...
            if remastered_fx_folder.is_dir():
                copy_remastered_textures(
                    remastered_fx_folder, out_path / "remastered-effects"
                )
    ...
    def extract_game_models() -> Path:
        ...
        for vanilla_key in _vanilla_keyblades():
            ...
            models = [
                f"{vanilla_key.model_name}{suffix}"
                for suffix in ["", "_NM", "_TR", "_WI"]
            ]
            KeybladeRandomizer.extract_keyblade(
                ...
                remastered_itempic=extracted_data_path
                / "remastered"
                / "itempic"
                / imd_name
                / "-0.dds",
                ...
                remastered_mdlx_folders=[
                    remastered_obj_path / f"{model}.mdlx" for model in models
                ],
                fx_files=[obj_path / f"{model}.a.us" for model in models],
                remastered_fx_folders=[
                    remastered_obj_path / f"{model}.a.us" for model in models
                ],
                remastered_sound_files=[
                    remastered_obj_path
                    / f"{model}.a.us/se/obj/{vanilla_key.sound_name}"
                    for model in models
                ],
            )
    ...
    def import_keyblade(packaged_file: str) -> str:
        ...
        if custom_visuals_path is None:
            raise GeneratorException(
                "No custom visuals path configured. Can't import keyblade."
            )
        ...
        for child in os.listdir(vanilla_keys_path):
            ...
            if vanilla_key_path.is_dir():
                ...
                if keyblade_json_path.is_file():
                    result.append(
                        ReplacementKeyblade(
                            vanilla_key_path.name, vanilla_key_path, keyblade_json_path
                        )
                    )
        ...
        for child in os.listdir(custom_keys_path):
            ...
            if custom_key_path.is_dir():
                ...
                if keyblade_json_path.is_file():
                    result.append(
                        ReplacementKeyblade(
                            custom_key_path.name, custom_key_path, keyblade_json_path
                        )
                    )
    ...
    def randomize_keyblades(
        setting: str, include_effects: bool, allow_duplicate_replacement: bool
        ...
        for vanilla_key in vanilla_keyblades:
            ...
            assets.extend(
                KeybladeRandomizer._keyblade_assets(
                    vanilla_key, replacement, "base", include_effects
                )
            )
            assets.extend(
                KeybladeRandomizer._keyblade_assets(
                    vanilla_key, replacement, "nm", include_effects
                )
            )
            assets.extend(
                KeybladeRandomizer._keyblade_assets(
                    vanilla_key, replacement, "tr", include_effects
                )
            )
            assets.extend(
                KeybladeRandomizer._keyblade_assets(
                    vanilla_key, replacement, "wi", include_effects
                )
            )
    ...
    @staticmethod
    def _itempic_assets(
        vanilla: VanillaKeyblade, replacement: ReplacementKeyblade
    ) -> list[Asset]:
        assets: list[Asset] = []
