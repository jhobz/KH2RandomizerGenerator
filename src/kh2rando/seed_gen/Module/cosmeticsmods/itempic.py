...
class Itempic:
    ...
    def __init__(
        self, itempic_id: str, item_name: str, types: list[str], custom: bool = False
    ):
        super().__init__()
...
class ItempicRandomizer:
    ...
    def bootstrap_itempic_file() -> Path:
        ...
        if not itempic_list_file_path.is_file():
            with open(
                itempic_list_file_path, mode="w", encoding="utf-8"
            ) as itempic_list_file:
                mapped = []
    ...
    def randomize_itempics(setting: str) -> list[Asset]:
        ...
        replacements_by_category: dict[str, list[tuple[bool, str]]] = (
            {}
        )  # (internal, path)

        if (
            setting == configDict.RANDOMIZE_IN_GAME_ONLY
            or setting == configDict.RANDOMIZE_ALL
        ):
            for category, game_files in ItempicRandomizer._collect_game_itempic_files(
                categorize
            ).items():
                if category not in replacements_by_category:
                    replacements_by_category[category] = []
                replacements_by_category[category].extend(
                    (True, file) for file in game_files
                )
        ...
        if (
            ...
            for (
                category,
                custom_files,
            ) in ItempicRandomizer._collect_custom_itempic_files(categorize).items():
                if category not in replacements_by_category:
                    replacements_by_category[category] = []
                replacements_by_category[category].extend(
                    (False, str(path)) for path in custom_files
                )
        ...
        for info in itempic_metadata:
            ...
            types: list[str] = [
                item_type.lower()
                for item_type in info["type"]
                if item_type.lower() in replacements_by_category
            ]
            if len(types) > 0:
                random.shuffle(types)

                for chosen_type in types:
                    itempics_for_chosen_type = replacements_by_category.get(
                        chosen_type, []
                    )

                    # Unlike the music rando we'll just always allow duplicate replacements here to simplify
                    if len(itempics_for_chosen_type) == 0:
                        refill_list = backup_files_by_categories.get(
                            chosen_type, []
                        ).copy()
    ...
    def replaceable_itempics() -> list[Itempic]:
        def consumable(itempic_id: str, item_name: str) -> Itempic:
            return Itempic(
                itempic_id=itempic_id,
                item_name=item_name,
                types=["Consumable", "Consumable", "Wild"],
            )
        ...
        def accessory(itempic_id: str, item_name: str) -> Itempic:
            return Itempic(
                itempic_id=itempic_id,
                item_name=item_name,
                types=["Accessory", "Accessory", "Wild"],
            )
        ...
        def synthesis(itempic_id: str, item_name: str) -> Itempic:
            return Itempic(
                itempic_id=itempic_id,
                item_name=item_name,
                types=["Synthesis", "Synthesis", "Wild"],
            )
        ...
        result = [
            ...
            Itempic(
                "015",
                "Dummy 23 (HP Up)",
                types=["StatIncrease", "StatIncrease", "Wild"],
                custom=True,
            ),
            Itempic(
                "016",
            ...
            Itempic(
                "018",
                "Dummy 26 (Armor Slot Up)",
                types=["StatIncrease", "StatIncrease", "Wild"],
                custom=True,
            ),
            Itempic(
                "019",
                "Dummy 27 (Accessory Slot Up)",
                types=["StatIncrease", "StatIncrease", "Wild"],
                custom=True,
            ),
            Itempic(
                "020",
                "Dummy 16 (Item Slot Up)",
                types=["StatIncrease", "StatIncrease", "Wild"],
                custom=True,
            ),
            ...
            Itempic(
                "091", "Dummy 13 (Royal Summons)", types=["RoyalSummons"], custom=True
            ),
            ...
            Itempic(
                "235", "Cursed Medallion", types=["OtherItem", "OtherItem", "Wild"]
            ),
            ...
            Itempic(
                "240", "Dummy 15 (Unknown Disk)", types=["UnknownDisk"], custom=True
            ),
            Itempic("241", "Secret Ansem Report", types=["AnsemReport"]),
            Itempic(
                "241", "Cursed Medallion", types=["OtherItem", "OtherItem", "Wild"]
            ),
    ...
    def _collect_custom_itempic_files(categorize: bool) -> dict[str, list[Path]]:
        ...
        if custom_visuals_path is not None:
            ...
            if item_pictures_path.is_dir():
                for child in [
                    str(category_file).lower()
                    for category_file in os.listdir(item_pictures_path)
                ]:
