...
class SeedModBuilder:
    ...
    def add_objective_randomization_mods(self, num_objectives_needed, objective_list):
        ...
        self.journal_txt.add_message(
            14260, en=convert_string_to_unicode("Objective List")
        )
        self.journal_txt.add_message(
            13945, en=convert_string_to_unicode("Reports/Objectives")
        )
        listText = ""
        counter = 1
        ...
        self.journal_txt.add_message(message_id=14261, en=listText)
    ...
    def add_sora_bdscript(self, item_ids, ability_ids, equipped):
        ...
        if len(ability_ids) != 0:
            ...
            for id in ability_ids:
                item_script += ability_string.replace("ITEM_ID", str(id)).replace(
                    "EQUIPPED", str(int(equipped))
                )
        if len(item_ids) != 0:
            for id in item_ids:
        ...
        with open(
            resource_path("static/starting/starting_inventory.bdscript"), "r"
        ) as infile:
            full_item_script = infile.read()

        full_item_script = full_item_script.replace("ITEM_LIST_GOES_HERE", item_script)
        starting_item_bdscript = _relative_mod_file(
            "starting_items/starting_inventory.bdscript"
        )
        starting_item_evt = _relative_mod_file(
            "starting_items/starting_item_event.script"
        )
        ...
        self.out_zip.write(
            resource_path("static/starting/starting_item_event.script"),
            starting_item_evt,
        )
    ...
    def add_base_messages(
        ...
        self.messages.add_message(
            20239, en=final_door_requirement_message, jp=final_door_requirement_message
        )
    ...
    def write_keyblade_locking_lua(self):
        keyblade_lua_name = _relative_mod_file(
            "keyblade_locking/F266B00B Keyblade Locking.lua"
        )
        assets = [
            {
    ...
    def write_goa_lua(self):
        ...
        self.out_zip.write(resource_path("static/F266B00B GoA ROM.lua"), goa_lua_name)
    ...
    def write_chest_visuals_assets(
        ...
        self.out_zip.write(
            resource_path("static/chests/remastered/equipment.dds"),
            equipment_source_name,
        )
...
class CosmeticsModAppender:
    ...
    def write_music_rando_assets(
        self, music_assets: list[Asset], music_replacements: dict[str, str]
    ):
        self.mod_yml.add_assets(music_assets)
        self._write_music_replacements(music_replacements)

    def write_keyblade_rando_assets(
        self, keyblade_assets: list[Asset], keyblade_replacements: dict[str, str]
    ):
    ...
    def write_rando_themed_texture_assets(self):
        ...
        goa_computer_source_name = _relative_mod_file(
            "textures/goa/randomizer-themed-computer.dds"
        )
        tt_computer_source_name = _relative_mod_file(
            "textures/tt-computer/randomizer-themed-computer-tt.dds"
        )
        data_door_source_name = _relative_mod_file(
            "textures/data-door/data-door-logo.dds"
        )
        final_arena_logo_source_name = _relative_mod_file(
            "textures/final-arena/randomizer-logo.dds"
        )
        ...
        self.out_zip.write(
            resource_path(
                "static/textures/tt-computer/randomizer-themed-computer-tt.dds"
            ),
    ...
    def _write_keyblade_replacements(self, replacements: dict[str, str]):
        if len(replacements) > 0:
            ...
            for original, replacement in replacements.items():
                keyblade_replacements_string += (
                    f"[{original}] was replaced by [{replacement}]\n"
                )
            self.out_zip.writestr(
                "keyblade-replacement-list.txt", keyblade_replacements_string
            )
    ...
    def _write_music_replacements(self, replacements: dict[str, str]):
        if len(replacements) > 0:
            ...
            self.out_zip.writestr(
                "music-replacement-list.txt", music_replacements_string
            )
