...
from .progressionPoints import ProgressionPoints
...
class RandomizerSettings:
    ...
    def __init__(
        ...
        self.disabledLocations: list[locationType] = [
            ...
            ]  # added creations here since we have all the subtypes covered
        ]
        ...
        self.min_double_stat_level = ui_settings.get(
            settingkey.DOUBLE_STAT_GROWTH_MIN_LEVEL
        )
        self.max_double_stat_level = ui_settings.get(
            settingkey.DOUBLE_STAT_GROWTH_MAX_LEVEL
        )
        ...
        self.starting_inventory_ids.extend(
            [
                magic.Fire.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_FIRE))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                magic.Blizzard.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_BLIZZARD))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                magic.Thunder.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_THUNDER))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                magic.Cure.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_CURE))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                magic.Magnet.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_MAGNET))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                magic.Reflect.id
                for _ in range(ui_settings.get(settingkey.STARTING_MAGIC_REFLECT))
            ]
        )
        self.starting_inventory_ids.extend(
            [
                misc.TornPages.id
                for _ in range(ui_settings.get(settingkey.STARTING_PAGES))
            ]
        )
        ...
        self.starting_visit_mode: StartingVisitMode = StartingVisitMode[
            ui_settings.get(settingkey.STARTING_VISIT_MODE)
        ]
        ...
        self.promiseCharmDepth = locationDepth(
            ui_settings.get(settingkey.PROMISE_CHARM_DEPTH)
        )
        ...
        self.boss_enemy_overrides: str = (
            boss_enemy_overrides
            if boss_enemy_overrides
            and (
                self.enemy_options["boss"] != "Disabled"
                or self.enemy_options["enemy"] != "Disabled"
            )
            else ""
        )
        ...
        self.chainLogicMaxLength: int = ui_settings.get(
            settingkey.MAX_CHAIN_LOGIC_LENGTH
        )
        ...
        self.keyblades_unlock_chests: bool = ui_settings.get(
            settingkey.KEYBLADES_LOCK_CHESTS
        )

        self.roxas_abilities_enabled: bool = ui_settings.get(
            settingkey.ROXAS_ABILITIES_ENABLED
        )
        self.disable_antiform: bool = (
            ui_settings.get(settingkey.DISABLE_FINAL_FORM)
            == DisableFinalOption.NO_ANTIFORM
        )
        self.disable_final_form: bool = (
            ui_settings.get(settingkey.DISABLE_FINAL_FORM)
            == DisableFinalOption.NO_FINAL
        )
        ...
        self.include_munny_pouches: bool = ui_settings.get(
            settingkey.MUNNY_IN_ITEM_POOL
        )
        ...
        final_door_requirement = FinalDoorRequirement[
            ui_settings.get(settingkey.FINAL_DOOR_REQUIREMENT)
        ]
        self.objective_rando: bool = (
            final_door_requirement is FinalDoorRequirement.OBJECTIVES
        )
        self.emblems: bool = final_door_requirement is FinalDoorRequirement.EMBLEMS
        self.num_objectives_needed = ui_settings.get(
            settingkey.OBJECTIVE_RANDO_NUM_REQUIRED
        )
        self.max_objectives_available = ui_settings.get(
            settingkey.OBJECTIVE_RANDO_NUM_AVAILABLE
        )
        ...
        if self.objective_pool_type == ObjectivePoolOption.BOSSES.name:
            self.available_objectives = [
                o
                for o in self.available_objectives
                if o.Type == ObjectiveList.ObjectiveType.BOSS
            ]
        elif self.objective_pool_type == ObjectivePoolOption.NOBOSSES.name:
            self.available_objectives = [
                o
                for o in self.available_objectives
                if o.Type == ObjectiveList.ObjectiveType.WORLDPROGRESS
                or o.Type == ObjectiveList.ObjectiveType.FIGHT
            ]
        elif self.objective_pool_type == ObjectivePoolOption.HITLIST.name:
            self.available_objectives = [
                o
                for o in self.available_objectives
                if o.Difficulty == ObjectiveList.ObjectiveDifficulty.LATEST
            ]
        ...
        if self.max_objectives_available > len(self.available_objectives):
            raise SettingsException(
                "Not enough enabled objectives for the requested available pool. Turn on more objectives or lower available"
            )
    ...
    def validateSettings(self):
        ...
        if "report" not in self.hintable_check_types:
            if (
                ...
                raise SettingsException(
                    f"The selected hint system {'Progression' if self.progression_hints else ''} {self.hintsType}  needs to have reports trackable"
                )
        ...
        if (
            self.objective_rando
            and self.max_objectives_available < self.num_objectives_needed
        ):
            raise SettingsException(
    ...
    def companion_exp(self) -> list[int]:
        return experienceValues.get_companion_exp(self.sora_exp_multiplier)
