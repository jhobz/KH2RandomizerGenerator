...
from . import version
from .Hints.HintOutput import HintOutput
from .Hints.HintUtils import (
    CommonTrackerInfo,
    HintUtils,
    JsmarteeHintData,
    PathHintData,
...
from .seedmod import SeedModBuilder
...
class Hints:
    ...
    def generator_journal_hints(
        ...
        if independent_hints:
            Hints.independent_journal_hints(
                location_item_data, journal_data, independent_hint_specific
            )
    ...
    def get_journal_hint(
        ...
        if locationCategory.WEAPONSLOT in location.LocationCategory:
            ...
            if keyblade_location is None:
                # Not in a world we're allowed to hint
                return None
            hintable_world = next(
                (
                    world
                    for world in keyblade_location.LocationTypes
                    if world in hintable_worlds
                ),
                None,
            )
            ...
            if independent_hint_specific:
                ...
                return (
                    f"{hinted_item_name} is in {world_text} (on {keyblade_item.name})."
                )
        else:
            hintable_world = next(
                (world for world in location.LocationTypes if world in hintable_worlds),
                None,
            )
            if hintable_world is None:
                # Not in a world we're allowed to hint
            ...
            if independent_hint_specific:
                return (
                    f"{hinted_item_name} is at {world_text} - {location.Description}."
                )
            else:
    ...
    def independent_journal_hints(
        ...
        for location, item_data in location_item_data:
            hint = Hints.get_journal_hint(
                location, item_data, location_item_data, independent_hint_specific
            )
            if hint is not None:
    ...
    @staticmethod
    def generate_hints_v2(
        randomizer: Randomizer, settings: RandomizerSettings
    ) -> HintData:
        ...
        if settings.emblems:
            hint_data["emblems"] = {
                "max_emblems_available": settings.max_emblems_available,
                "num_emblems_needed": settings.num_emblems_needed,
            }
        ...
        if settings.objective_rando:
            ...
            objective_tuples = [
                {"category": l_i[0].LocationCategory, "location_id": l_i[0].LocationId}
                for l_i in location_item_tuples
                if l_i[0].Description in objective_locations
            ]
            # remove duplicates
            objective_tuples = [
                dict(t) for t in {tuple(d.items()) for d in objective_tuples}
            ]
    ...
    @staticmethod
    def write_hint_text(
        hint_data: HintData, mod: SeedModBuilder, journal_hints_spoiler: dict[str, str]
    ):
        def convert_string_to_unicode(string: str, newlines: int = 0):
            return (
                "".join(
                    r"\u{:04X}".format(ord(character))
                    for character in textwrap.fill(string, width=30)
                )
                + "NEWLINE" * newlines
            )
        ...
        for report_number in range(0, 13):
            ...
            if "ProgressionSettings" in hint_data:
                if hint_data["hintsType"] == HintType.POINTS:
                    ...
                    hint_text.append(
                        f"{hinted_world_text(hinted_world)} has {report_data['itemName']}."
                    )
            else:
                if hint_data["hintsType"] == HintType.JSMARTEE:
                    ...
                    if check_count == 1:
                        hint_text.append(
                            f"{hinted_world_text(hinted_world)} has {check_count} important check."
                        )
                    else:
                        hint_text.append(
                            f"{hinted_world_text(hinted_world)} has {check_count} important checks."
                        )
            ...
            for hint_text_instance in hint_text:
                ...
                encoded_text += convert_string_to_unicode(
                    hint_text_instance, newlines=1
                )
            spoiler_text += "\n"
            ...
            if "Location" in report_data:
                ...
                if isinstance(report_location, locationType):
                    report_location_text = HintUtils.location_hint_user_friendly_text(
                        report_location
                    )
            ...
            mod.journal_txt.add_message(message_id=report_message_id, en=encoded_text)
