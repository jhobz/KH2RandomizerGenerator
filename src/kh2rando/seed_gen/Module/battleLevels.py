...
class BtlvViewer:
    def __init__(self):
        ...
        if self.goa_btlv:
            ...
            self.visit_flags = {}
            self.visit_flags[locationType.STT] = [
                (2, 0x00010),
                (2, 0x00020),
                (2, 0x00040),
            ]
            self.visit_flags[locationType.TT] = [
                (2, 0x00100),
                (2, 0x00200),
                (2, 0x00800),
            ]
            self.visit_flags[locationType.HB] = [
                (4, 0x00010),
                (4, 0x00080),
                (4, 0x00200),
            ]
            ...
            self.visit_flags[locationType.DC] = [
                (12, 0x00010),
                (13, 0x00010),
                (13, 0x00020),
            ]
        ...
        if self.random_option == BattleLevelOption.NORMAL.name:
            ...
            if battle_level_offset is None:
                raise BackendException(
                    "Trying to offset battle levels without a provided offset"
                )
            self._make_btlv_vanilla()
            ...
            battle_level_random_min, battle_level_random_max = (
                battle_level_random_min_max
            )
    ...
    def write_modifications(self) -> bytearray:
        for x in range(20):
            offset = 8 + 32 * x
            for y in range(8, 32):
                self.binaryContent[offset + y] = number_to_bytes(self.flags[x][y - 8])[
                    0
                ]
    ...
    def _variance_btlv(self):
        ...
        if level_range is None:
            raise BackendException(
                "Trying to range battle levels without a provided range"
            )
        elif level_range == 0:
            return
        for world, visit_flag_list in self.visit_flags.items():
            ...
            for visit_number in range(len(visit_flag_list)):
                ...
                self._set_battle_level(
                    world,
                    visit_flag_list[visit_number][0],
                    visit_number,
                    current_btlvs[visit_number] + btlv_change,
                )
    ...
    def _pure_random_btlv(self, btlv_minimum: int, btlv_maximum: int):
        for world, visit_flag_list in self.visit_flags.items():
            for visit_number in range(len(visit_flag_list)):
                ...
                self._set_battle_level(
                    world, visit_flag_list[visit_number][0], visit_number, btlv_change
                )
    ...
    def _shuffle_btlv(self):
        ...
        for world, visit_flag_list in self.visit_flags.items():
            for visit_number in range(len(visit_flag_list)):
                ...
                self._set_battle_level(
                    world, visit_flag_list[visit_number][0], visit_number, new_level
                )
    ...
    def _scale_btlv(self, scaled_level):
        for world, visit_flag_list in self.visit_flags.items():
            ...
            for visit_number in range(len(visit_flag_list)):
                new_level = int(
                    (current_btlvs[visit_number] * 1.0 / current_btlvs[-1])
                    * scaled_level
                )
                self._set_battle_level(
                    world, visit_flag_list[visit_number][0], visit_number, new_level
    ...
    def _set_battle_level(self, world, world_index, visit_number, new_level):
        ...
        if not self.goa_btlv and prev_visit_flags[0] == current_visit_flags[0]:
            ...
            self._set_level(
                world_index, (current_visit_flags[0], changed_flags), delta_btlv
            )
        else:
    ...
    def _set_level(self, world_index, flags_to_set, number_to_set):
        ...
        while flags_to_set > 0:
            if not skip_9 and flag_index == 9:
                ...
                continue
            if flags_to_set % 2 == 1:
                self.flags[flag_index][world_index] = (
                    0 if added_number else number_to_set
                )
                added_number = True
            flag_index += 1
            flags_to_set = flags_to_set >> 1
