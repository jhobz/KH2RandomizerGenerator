...
from .depths import ItemDepths
from .modifier import SeedModifier
from .weighting import LocationWeights
...
class Randomizer:
    def __init__(self, settings: RandomizerSettings, progress_bar_vis: bool = False):
        ...
        self.promise_charm_depths = ItemDepths(
            settings.promiseCharmDepth, self.master_locations
        )
    ...
    def assign_level_stats(self, settings: RandomizerSettings):
        ...
        for index, location in enumerate(locations):
            if index != 0:
                ...
                if (
                    location.LocationId in excluded_levels
                    and location.LocationId in double_stat_levels
                ):
    ...
    def initial_item_pool(self, settings: RandomizerSettings) -> list[KH2Item]:
        ...
        for item_id_to_remove in item_ids_to_remove:
            item_to_remove = next(
                (item for item in item_pool if item.Id == item_id_to_remove), None
            )
        ...
        if settings.shop_unlocks > 0:
            ...
            visit_unlock_pool = list(
                set([i for i in item_pool if i.ItemType == itemType.STORYUNLOCK])
            )
    ...
    def assign_sora_items(self, settings: RandomizerSettings):
        ...
        if settings.emblems:
            item_pool.extend(
                [Items.emblemItem() for _ in range(settings.max_emblems_available)]
            )
        ...
        for loc_with_vanilla in locations_with_vanilla_items:
            ...
            for vanilla_item_id in loc_with_vanilla.VanillaItems:
                ...
                if True:  # vanilla_item.ItemType not in loc_with_vanilla.InvalidChecks:
                    ...
                    if self.assign_item(
                        loc_with_vanilla, vanilla_item, override_invalid=True
                    ):
        ...
        if settings.objective_rando:
            ...
            objective_pool = [
                o for o in objective_pool if o.Location in valid_location_descriptions
            ]
            ...
            if (
                ...
                if settings.as_data_split:
                    ...
                    objective_pool = [
                        o for o in objective_pool if "Defeat AS" not in o.Name
                    ]
                else:
                    # remove duplicates for Datas
                    objective_pool = [
                        o
                        for o in objective_pool
                        if not any(
                            t in o.Name
                            for t in [
                                "Data Vexen",
            ...
            if settings.objective_pool_type == ObjectivePoolOption.HITLIST.name:
                ...
                objective_pool = [
                    o for o in objective_pool if "Level 7" not in o.Name
                ] + form_choice

            if len(objective_pool) < settings.max_objectives_available:
                raise SettingsException(
                    f"Not enough objective locations ({len(objective_pool)}) available to allow the max number of objectives ({settings.max_objectives_available}) to be placed."
                )
            ...
            all_form_objectives = (
                form_3_objectives + form_5_objectives + form_7_objectives
            )
            ...
            if settings.max_objectives_available > len(non_form_objectives):
                non_form_objectives.extend(
                    all_form_objectives[
                        0 : (
                            settings.max_objectives_available - len(non_form_objectives)
                        )
                    ]
                )
            else:
                # pick first 3
                non_form_objectives.extend(
                    all_form_objectives[0 : min(3, len(all_form_objectives))]
                )
            objective_pool = non_form_objectives

            # pick a number of objectives
            self.objectives = random.sample(
                objective_pool, k=settings.max_objectives_available
            )
            ...
            objective_locations = [
                ...
                if v.Description in picked_objectives_location_names
            ]
            for o in objective_locations:
                locations_to_remove = self.randomly_assign_single_item(
                    Items.objectiveItem(), [o]
                )
    ...
    def assign_chain_logic(
        self, settings: RandomizerSettings, item_pool, valid_locations
    ):
        ...
        acquired_items, accessible_locations = self.get_accessible_locations(
            valid_locations, validator, simulated_growth + shop_item_ids
        )
        acquired_items = (
            acquired_items + self.starting_item_ids + simulated_growth + shop_item_ids
        )
        last_unlocked_sphere = accessible_locations

        world_names_to_unlock = [
            w
            for w in item_locking_ids_per_world.keys()
            for _ in item_locking_ids_per_world[w]
        ]
        random.shuffle(world_names_to_unlock)
        world_names_to_unlock = [
            w for w in world_names_to_unlock if w in settings.enabledLocations
        ]

        proof_nonexistence_assignment = self.assignment_for_item_id(
            proof.ProofOfNonexistence.id
        )
        ...
        if proof_nonexistence_assignment is not None:
            ...
            proof_world = list(
                set(proof_nonexistence_assignment.location.LocationTypes).intersection(
                    item_locking_ids_per_world.keys()
                )
            )[0]
            ...
            for w in world_progression:
                if validator.is_location_available(
                    self.starting_item_ids + items_needed,
                    proof_nonexistence_assignment.location,
                ):
                    # uh, bad chain
                    raise GeneratorException(
                        "Chain logic couldn't make a seed that ended on proof of nonexistence. Try changing proof depths."
                    )
            ...
            last_index = [
                i
                for i in range(len(world_names_to_unlock))
                if world_names_to_unlock[i] == proof_world
            ][-1]
        ...
        while item_depth < min_item_depth or (
            item_depth < max_item_depth and random.random() < 0.75
        ):  # 25% chance of breaking early
            ...
            sphere_1 = [
                loc
                for loc in valid_locations
                if locationType.Level not in loc.LocationTypes
                and validator.is_location_available(acquired_items + chosen_checks, loc)
                and loc not in accessible_locations
            ]
            if settings.extended_placement_logic:
                sphere_1 = [
                    loc
                    for loc in sphere_1
                    if loc.Description != hb.CheckLocation.DataDemyxApBoost
                ]
            ...
            if len(sphere_1) and len(last_unlocked_sphere) >= len(chosen_checks):
                ...
                for i in chosen_checks:
                    ...
                    if i_data is not None:
                        ...
                        try:
                            locations_to_remove = self.randomly_assign_single_item(
                                i_data, last_unlocked_sphere
                            )
                        except Exception:
                            raise GeneratorException(
                                f"Tried to assign {i_data} to {last_unlocked_sphere} and failed"
                            )
                        for l in locations_to_remove:
                            valid_locations.remove(l)
                ...
                if chosen_world not in settings.enabledLocations:
                    ...
                    len(chosen_checks) == 1
                    and chosen_checks[0]
                    in [k.id for k in keyblade.get_locking_keyblades()]
                    and settings.keyblades_unlock_chests
                ):
                    pop_world_from_list()
                else:
                    # print(chosen_world)
        ...
        if item_depth < min_item_depth:
            raise GeneratorException(
                "Couldn't generate a chain long enough, are enough worlds on?..."
            )

        i_data = next(
            (it for it in item_pool if it.Id == proof.ProofOfNonexistence.id), None
        )
        if i_data is not None:
            # assign this item somewhere
            locations_to_remove = self.randomly_assign_single_item(
            ...
            acquired_items, accessible_locations = self.get_accessible_locations(
                valid_locations, validator, simulated_growth
            )
            ...
            if proof.ProofOfNonexistence.id not in acquired_items:
                raise GeneratorException(
                    "Couldn't access proof of nonexistence in chain logic"
                )
    ...
    def create_available_location_space(self, settings, item_pool, valid_locations):
        ...
        sphere_0_check = 30
        num_available_locations_needed_to_allow_random_assignment = (
            125  # TODO(zak) this is arbitrary and may need tuning
        )
        while sphere_0_check > 0:
            # calculate the available checks
            _, sphere_0 = self.get_accessible_locations(
                valid_locations, validator, shop_item_ids
            )
            if (
                len(sphere_0)
                < num_available_locations_needed_to_allow_random_assignment
                ...
                unlocks = (
                    []
                    + [s.id for s in storyunlock.all_story_unlocks()]
                    + [k.id for k in keyblade.get_locking_keyblades()]
                )
                random.shuffle(unlocks)
                for u in unlocks:
                    i_data = next((it for it in item_pool if it.Id == u), None)
                    if i_data is not None:
                        _, sphere_1 = self.get_accessible_locations(
                            valid_locations, validator, shop_item_ids + [i_data.Id]
                        )
                        if len(sphere_1) > len(sphere_0):
                            ...
                            break
                sphere_0_check -= (
                    1  # make sure this doesn't ever cause a softlock again
                )
            else:
                sphere_0_check = 0
    ...
    def get_accessible_locations(self, valid_locations, validator, aux_items=None):
        ...
        while found_new_item:
            ...
            sphere_0 = [
                ...
                for loc in valid_locations
                if validator.is_location_available(
                    self.starting_item_ids + acquired_items + aux_items, loc
                )
            ]
            # get already assigned items from available locations
            for assignment in self.assignments:
                if (
                    assignment.location.LocationCategory
                    is not locationCategory.WEAPONSLOT
                    and assignment.location not in acquired_item_locations
                    and validator.is_location_available(
                        self.starting_item_ids + acquired_items + aux_items,
                        assignment.location,
                    )
                ):
                    if (
                        assignment.location.Description
                        != stt.CheckLocation.StruggleWinnerChampionBelt
                    ):
    ...
    def assign_plando_like_items(self, settings, item_pool, valid_locations):
        ...
        if restricted_proofs:
            remaining_proofs = [
                i for i in item_pool if i.ItemType in proof.proof_item_types()
            ]
            # pick N valid locations for these items
            valid_for_proofs = [
                loc for loc in valid_locations if self.proof_depths.is_valid(loc)
            ]
            ...
            if (
                ...
                valid_for_proofs = [
                    loc
                    for loc in valid_for_proofs
                    if locationType.AS not in loc.LocationTypes
                ]

            good_choices = False
            while not good_choices:
                good_choices = True
                chosen_locations = random.sample(
                    valid_for_proofs, k=len(remaining_proofs)
                )
        ...
        if restricted_promise_charm:
            promise_charm_item_list = [
                i for i in item_pool if i.ItemType is misc.PromiseCharm.type
            ]
            # pick N valid locations for these items
            valid_for_promise_charm = [
                loc
                for loc in valid_locations
                if self.promise_charm_depths.is_valid(loc)
            ]
            chosen_locations = random.sample(
                valid_for_promise_charm, k=len(promise_charm_item_list)
            )
            for index, c in enumerate(chosen_locations):
        ...
        if restricted_story:
            remaining_unlocks = [
                i for i in item_pool if i.ItemType is itemType.STORYUNLOCK
            ]
            # pick N valid locations for these items
            valid_for_unlocks = [
                loc for loc in valid_locations if self.story_depths.is_valid(loc)
            ]
        ...
        if restricted_reports:
            ...
            # pick N valid locations for these items
            valid_for_reports = [
                loc for loc in valid_locations if self.report_depths.is_valid(loc)
            ]
    ...
    def randomly_assign_single_item(self, item, location_pool):
        ...
        if len(location_pool) == 0:
            raise CantAssignItemException(
                f"Ran out of locations to assign items {item}"
            )
        ...
        while True:
            ...
            random_location: KH2Location = random.choices(location_pool, weights)[0]
    ...
    def compute_location_weights(
        ...
        result = [loc_weights.get_weight(item.ItemType, loc) for loc in location_pool]
        return result
