...
def mimic_item_placement_difficulty(
    seed_settings: SeedSettings, difficulty: itemDifficulty
):
...
def spoilerHints(seed_settings: SeedSettings):
    ...
    seed_settings.set(
        ...
        seed_settings.get(settingkey.HINTABLE_CHECKS) + ["report"],
    )
...
def keybladeLocking(seed_settings: SeedSettings):
    ...
    seed_settings.set(
        settingkey.HINTABLE_CHECKS,
        seed_settings.get(settingkey.HINTABLE_CHECKS) + ["keyblade"],
    )
...
dailyModifiers = [
    ...
    DailyModifier(
        ...
        local_modifier=lambda settings: settings.set(
            settingkey.STARTING_VISIT_MODE, StartingVisitMode.FIRST.name
        ),
    ),
    DailyModifier(
        name="Glass Cannon",
    ...
    DailyModifier(
        ...
        local_modifier=lambda settings: mimic_item_placement_difficulty(
            settings, itemDifficulty.SLIGHTLY_EASY
        ),
    ),
    DailyModifier(
        ...
        categories={"placement"},
        local_modifier=lambda settings: mimic_item_placement_difficulty(
            settings, itemDifficulty.SLIGHTLY_HARD
        ),
    ),
    DailyModifier(
...
dailyHardModifiers = [
    ...
    DailyModifier(
        ...
        categories={"placement"},
        local_modifier=lambda settings: mimic_item_placement_difficulty(
            settings, itemDifficulty.VERYHARD
        ),
    ),
    DailyModifier(
