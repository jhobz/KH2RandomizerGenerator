...
from ..newRandomize import ItemAssignment
    ...
    def __init__(
        ...
        for location, item in location_item_tuples:
            ...
            if isinstance(inventory_item, AnsemReport):
                ...
                self.report_information[inventory_item.report_number][
                    "FoundIn"
                ] = world_of_location
                ...
                if item.ItemType is itemType.PROOF_OF_PEACE:
                    ...
                    for vanilla_world in item_to_vanilla_world[inventory_item]:
                        if world_of_location in hintable_worlds:
                            self.path_breadcrump_map[vanilla_world].add(
                                world_of_location
                            )
            ...
            if item.ItemType in important_checks or item.Name in important_checks:
                if locationType.Level in location.LocationTypes:
                    ...
                    if dream_weapon_matters:
                        self.level_checks[locationCategory.LEVEL]["Staff"][
                            offsets.get_staff_level(max_level, location.LocationId)
                        ] = item.Id
                        self.level_checks[locationCategory.LEVEL]["Shield"][
                            offsets.get_shield_level(max_level, location.LocationId)
                        ] = item.Id
                ...
                if locationType.FormLevel in location.LocationTypes:
                    for form_category in self.level_checks.keys():
                        if form_category is location.LocationCategory:
                            self.level_checks[form_category][
                                location.LocationId
                            ] = item.Id
...
class PathHintData:
    def __init__(
        ...
        points_to_connection = (
            poc_world and world_to_hint in world_items.path_breadcrump_map[poc_world]
        )
        pop_world = world_items.proof_of_peace_world
        points_to_peace = (
            pop_world and world_to_hint in world_items.path_breadcrump_map[pop_world]
        )
        pon_world = world_items.proof_of_nonexistence_world
        points_to_nonexistence = (
            pon_world and world_to_hint in world_items.path_breadcrump_map[pon_world]
        )
...
class HintUtils:
    ...
    @staticmethod
    def update_disabled_worlds_on_tracker(
        settings: RandomizerSettings,
    ) -> list[locationType]:
        ...
        if locationType.OC in exclude_list and (
            locationType.OCCups not in exclude_list
        ):
    ...
    def point_hint_report_assignment(
        ...
        for report_number in range(1, 14):
            for p in point_data:
                if (
                    ...
                    or world_items.report_information[report_number]["FoundIn"]
                    != p.world
                ):
                    for cand in p.candidate_hints():
                        report_number_to_world_to_item.append(
                            (report_number, p.world, cand)
                        )
        ...
        def pick_item(
            ...
            filtered_list = [
                (r, w, i)
                for r, w, i in input_data
                if w == input_world and r == input_report_num
            ]
        ...
        def reduce_list(
            ...
            filtered_list = [
                (r, w, i) for r, w, i in input_data if r != input_report_num
            ]
            filtered_list = [
                (r, w, i)
                for r, w, i in filtered_list
                if input_world != w or input_item != i
            ]
    ...
    def jsmartee_hint_report_assignment(
        ...
        def reduce_list(
            ...
            filtered_list = [
                (r, w, c) for r, w, c in input_data if r != input_report_num
            ]
        ...
        proof_worlds = [
            proof_world
            for proof_world in proof_worlds
            if proof_world in HintUtils.hintable_worlds()
        ]
        priority_worlds = []
        ...
        for _ in range(100):
            ...
            remaining_worlds = [
                world for world in hintable_worlds if world not in selected_worlds
            ]
            ...
            for index, world in enumerate(selected_worlds):
                ...
                if selected_world in proof_worlds:
                    ...
                    if (
                        report_world not in priorities
                        and report_world != "Garden of Assemblage"
                    ):
    ...
    def path_hint_assignment(
        ...
        for _ in range(50):
            ...
            if progression_hints:
                ...
                all_data = (
                    [p for p in all_data if p.num_items > 0]
                    + [
                        p
                        for p in all_data
                        if p.num_items == 0 and p.world in hintable_worlds
                    ]
                    + [
                        p
                        for p in all_data
                        if p.num_items == 0 and p.world not in hintable_worlds
                    ]
                )
